...
const BUILDS_DIR = path.resolve(ROOT, '..', 'Builds');
...
async function main() {
  ...
  console.log('--- Step 4: Copy to Builds folder ---');
  const installer = path.join(DIST, 'Zestok.exe');
  if (fs.existsSync(installer)) {
    fs.mkdirSync(BUILDS_DIR, { recursive: true });
    const dest = path.join(BUILDS_DIR, 'Zestok.exe');
